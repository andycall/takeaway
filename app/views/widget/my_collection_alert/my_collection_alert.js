define(['jquery'], function($){
    ...
    $(".add").on('click',function(){
        ...
        $(".collection-modal input").each(function () {
            $(this).is(":checked") && (
                n = $(this).attr("shop_id"),
                cancel_collection[n] = new Array(2),
                cancel_collection[n]["shop_id"] = n,
                cancel_collection[n]["place_id"] = $(this).attr("place_id")
            );
        });
    });
        ...
        switch (e_class){
            ...
            case "btn btn-yellow":
                ...
                $(".collection-modal input").each(function () {
                    $(this).is(":checked") && (
                        n = $(this).attr("shop_id"),
                            add_collection[n] = new Array(2),
                            add_collection[n]["shop_id"] = n,
                            add_collection[n]["place_id"] = $(this).attr("place_id")
                    );
                });

                add_collection.forEach(function (value, index) {
                    if(cancel_collection[index]){
                        cancel_collection.splice(index, 1);
                        add_collection.splice(index, 1);
                    }else{
                        add_collection[n] = new Array(2);
                        add_collection[n]["shop_id"] = n;
                        add_collection[n]["place_id"] = $(this).attr("place_id");
